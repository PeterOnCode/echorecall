...
- **FR-016**: With one or more recordings selected, users MUST be able to apply a bulk tag edit: the user picks one free-text editable tag field — Title, Artist, Album, Comment, Date, Track Number, Genre, Album Artist, Composer, Encoded-By, or Notes — and a value, and the system overwrites that field across the entire selection via the existing retag behavior, then reports how many succeeded and surfaces any failures. Name/filename is excluded, as applying one filename to many recordings would force rename collisions; the typed/multi-value fields (BPM, Rating, Language) are also excluded and stay per-recording — one shared free-text input cannot carry their integer/multi-value semantics cleanly.
...
- **FR-029** (R-TAGS): The tag set MUST be extended to support editing Notes, Encoded-By, Album Artist, Composer, BPM, and Rating by extending the tag metadata model and the tag read/write mapping. Each extra field MUST be written to the audio file as its native ID3 frame (Encoded-By→TENC, Album Artist→TPE2, Composer→TCOM, BPM→TBPM, Notes→a user-text/`customText` frame) — **except Rating, which persists only in the `tags_extra` JSON store**: taglib-wasm's property-map write path has no POPM support, so embedding the frame would require frame-level ID3 editing (deferred; the 0–5 ↔ POPM 0–255 byte mapping is kept in the shared core, `src/core/tagging/rating.ts`, for the future import/file-sync features). All extra fields are mirrored in the **existing `tags_extra` JSON store** for the SQLite row. Field formats: BPM is a non-negative integer; **Rating is a 0–5 integer (stars)**; the others are free text. This MUST NOT add a SQL column or require a migration, MUST NOT change generation behavior, and MUST remain reachable from the shared core (not introduced in the presentation layer).
