...
// captured POST body carries the derived Title (the text chopped at its first line break, then
// the first 120 chars of that line, ellipsised when longer) and Track (the row's 1-based queue
// position).
...
describe('generate-next derived Title + Track reach the API (007)', () => {
  it('sends the first-120-char title and the 1-based queue position as track', async () => {
    ...
    // Title: short text used as-is; long text truncated to 120 chars + ellipsis.
    expect(first.metadata?.title).toBe(short)
    expect(second.metadata?.title).toBe(`${'x'.repeat(120)}…`)
  ...
  it('counts the derived track up from the start number set in the action bar', async () => {
    const wrapper = await mountSuspended(GenerateNextPage)
    await drainMount()

    // Set the first track number to 5, then queue two rows.
    const startInput = wrapper.find('[data-test="start-track-input"]')
    await startInput.setValue('5')
    await startInput.trigger('blur')
    await flushPromises()

    await addTyped(wrapper, 'first')
    await addTyped(wrapper, 'second')

    await wrapper.find('[data-test="action-generate"]').trigger('click')
    for (let i = 0; i < 8; i++) await flushPromises()

    const first = postedBodies.find((b) => b.text === 'first')!
    const second = postedBodies.find((b) => b.text === 'second')!
    expect(first.metadata?.track).toBe('5')
    expect(second.metadata?.track).toBe('6')

    wrapper.unmount()
  })

  it('chops a multi-line entry at its first line break for the derived title', async () => {
    const wrapper = await mountSuspended(GenerateNextPage)
    await drainMount()

    const multiline = 'First line of the script\nSecond line stays out of the title'
    await addTyped(wrapper, multiline)

    await wrapper.find('[data-test="action-generate"]').trigger('click')
    for (let i = 0; i < 8; i++) await flushPromises()

    const sent = postedBodies.find((b) => b.text === multiline)!
    expect(sent.metadata?.title).toBe('First line of the script')
